...
CreateConVar("epoe_version", "2.62-jom", FCVAR_NOTIFY)
...
function AddSub(pl)
	if not pl or not pl.IsValid or not pl:IsValid() or not pl:IsPlayer() then return end
	local uid = pl:UserID()
...
-- Now you can print epoe table, not this table.
function GetTable()
	return Messages
end
...
function PushPayload(flags, text, msgc_col)
	...
	while txt and txt ~= "" do
		...
		-- let's stop here. You've done well enough...
		if i > 63 * 1024 then
			EnableTick()
			Messages:clear()
			InEPOE = false
			...
			Messages:push{
				flag = IS_EPOE,
				msg = "Cancelling messages, too many iterations."
			}
...
function OnTick()
	...
	if HasNoSubs then
		Messages:clear()
	elseif not HitMaxQueue() and Messages:len() > 0 then
	...
	if not module_loaded and enginespew_loaded then
		...
		hook.Add("EngineSpew", TagHuman, function(a, msg, c, d, col_r, col_g, col_b)
			...
			if incoming_clienterr then
				...
				local newmsg = tostring(msg:match("%[ERROR%] (.+)") or msg) --[[not  epoe_client_traces:GetBool() and msg:match("%[ERROR%] (.-)\n") or]]
			...
			-- Does it change if it's a workshop error? If it does, we're fucked.
			if msg:sub(1, 9) == "\n[ERROR] " then
				msg = msg:sub(10, -1)
				local newmsg = msg --[[not epoe_server_traces:GetBool() and msg:match("(.-)\n") or]]
				OnLuaError(newmsg)
